...
```{r horse-table, echo=FALSE, message = FALSE}
...
Additionally, note that in Table \@ref(tab:horse-table) we consider the sample standard deviation of the number of horses during the same time span as Figure1.

```{r horse-prov-pop, fig.cap = "Horse populations for the province with the largest standard deviation", out.width="50%", echo=FALSE}
knitr::include_graphics("../../results/horse_pop_plot_largest_sd.png")
```

In Figure \@ref(fig:horse-prov-pop) we zoom in and look at the province of `r largest_sd`, which had the largest spread of values in terms of standard deviation.
